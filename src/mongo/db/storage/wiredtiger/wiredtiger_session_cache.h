 ...
 * This contains an instance of a tag integer and a pointer to the data
 * structure
 * that is to be stored. The tag value is incremented ever time an object is
 * popped
 * this avoids the ABA problem by making this object different every time the
 * we pop from the stack
 */
template <class T> class TaggedAtomicWrapper {
public:
...
public:
   ...
   * @param epoch In which session cache cleanup epoch was this session
   *instantiated. Value
   *          of -1 means that this value is not necessary since the session
   *will not be
   *          cached.
   */
  ...
  /**
   * For "metadata:" cursors. Guaranteed never to collide with genCursorId()
   * ids.
   */
...
private:
  ...
  // The tag value is incremented every time the object is popped. This prevents
  // ABA
  uint64_t _tag = 0;
};
...
private:
  ...
  // Regular operations take it in shared mode. Shutdown sets the _shuttingDown
  // flag and
  // then takes it in exclusive mode. This ensures that all threads, which would
  // return
  ...
  // The sessions are stored as a linked list stack. So we need to track the
  // head
